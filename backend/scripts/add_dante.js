...
async function addUser() {
    try {
        ...
        const userData = {
            email: 'dante@premoldadosthora.com.br',
            passwordHash: 'TanderaThora@2026', 
            name: 'Dante - Thora',
            serverIp: 'https://apithora.tanderatech.com.br:3000'
        };

        // Verifica se já existe (busca pelo novo ou pelo antigo)
        const existingUser = await User.findOne({
            $or: [
                { email: userData.email },
                { email: 'dante@premoldsdosthora.com.br' }
            ]
        });
